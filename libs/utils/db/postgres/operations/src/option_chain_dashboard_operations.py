...
class OptionChainDashboardOperations(BaseOperations[OptionChainIntervalSummary]):
    ...
    def _compute_custom_pcrs(
        ...
        call_total_atm = cls._sum_range(
            strike_rows, atm_index + 1, atm_index + 1, "call_oi_change"
        )
        put_total_atm = cls._sum_range(
            strike_rows, atm_index - 1, atm_index - 1, "put_oi_change"
        )

        # Strength PCR: CALL at ATM and 4 strikes ABOVE, PUT at ATM and 4 strikes BELOW
        call_total_strength = cls._sum_range(
            strike_rows, atm_index, atm_index + 4, "call_oi_change"
        )
        put_total_strength = cls._sum_range(
            strike_rows, atm_index - 4, atm_index, "put_oi_change"
        )
