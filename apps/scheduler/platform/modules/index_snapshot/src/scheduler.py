...
log = CustomLogger("IndexSnapshotScheduler")
...
class IndexSnapshotScheduler:
    ...
    async def tick(self):
        ...
        try:
            if not is_market_open_now():
                ...
                logger.info(
                    "Index scheduler tick skipped - market closed at this time - "
                    f"now_utc: {now_utc}, "
                    f"now_ist: {now_ist}, "
            ...
            logger.info(
                "Index scheduler tick completed - "
                f"processed_indices: {result.get('processed_indices')}, "
    ...
    async def start(self):
        ...
        latest_captured_at = (
            await RuntimeIndexSnapshotService.get_latest_captured_at_for_today_ist()
        )
        ...
        self._started = True
        logger.info(
            "Index scheduler startup decision - "
        ...
        logger.info(
            color_string(
                "Index snapshot scheduler started - "
        ...
        if run_immediately_on_startup:
            logger.info(
                "Running immediate startup index snapshot because computed candidate time was already in the past."
            )
            await self.tick()
    ...
    def stop(self):
        ...
        self._started = False
        logger.info("Index snapshot scheduler stopped")
